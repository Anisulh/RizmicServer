...
export async function getFriends(req: Request, res: Response) {
  ...
  const friendships = await Friends.find({
    ...
    status: 'accepted',
  }).populate('requester recipient', 'firstName lastName profilePicture').lean<IFriendship[]>();
...
export async function getFriendRequests(req: Request, res: Response) {
    ...
    const friendRequests = await Friends.find({
        recipient: _id,
        status: 'pending',
    }).populate('requester', 'firstName lastName email profilePicture').lean();
...
export async function sendFriendRequest(req: Request, res: Response) {
    ...
    if (_id === userId) {
       throw new AppError({ message: 'You cannot send a friend request to yourself', httpCode: 400 });
    }
    ...
    if (friendRequest) {
        throw new AppError({ message: 'Friend request already sent', httpCode: 400 });
    }
...
export async function acceptFriendRequest(req: Request, res: Response) {
    ...
    if (!friendRequest) {
        throw new AppError({ message: 'Friend request not found', httpCode: 404 });
    }
